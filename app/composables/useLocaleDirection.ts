...
export function useLocaleDirection(options: UseLocaleDirectionOptions = {}) {
  ...
  const activeLocale = computed(() =>
    (locales.value as LocaleItem[]).find((l: LocaleItem) =>
      typeof l === "string"
        ? l === locale.value
        : l.code === locale.value
    )
  ...
  const direction = computed<"rtl" | "ltr">(() =>
    activeLocale.value &&
    typeof activeLocale.value !== "string" &&
    activeLocale.value.dir === "rtl"
      ? "rtl"
      : "ltr"
  );
  ...
  const iconByDirection = <T>(rtlValue: T, ltrValue: T) =>
    computed(() => (isRtl.value ? rtlValue : ltrValue));
