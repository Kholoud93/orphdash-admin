...
<template>
  <div
    v-if="props.notificationsOpen"
    class="fixed inset-0 z-50 flex items-start justify-end bg-foreground/30 p-4 backdrop-blur-sm"
  ...
  >
    <div class="mt-14 w-full max-w-sm rounded-2xl border bg-card p-5 shadow-xl" @click.stop>
      <div class="mb-3 flex items-center justify-between gap-2">
        ...
        <button
          type="button"
          class="rounded-lg border p-1.5 hover:bg-muted"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <span
              ...
              class="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-semibold uppercase text-primary"
            >
              {{ $t("shell.new") }}
            </span>
          </div>
          ...
          <p class="mt-2 text-[11px] text-muted-foreground">
            {{
              notification.time
                ? seedLabels.notificationField(notification.id, "time", notification.time)
                : ""
            }}
          </p>
        </li>
      ...
      <NuxtLink
        ...
        class="mt-4 inline-flex w-full items-center justify-center rounded-xl border px-3 py-2 text-sm font-medium hover:bg-muted"
        @click="emit('closeNotifications')"
      >
        {{ $t("shell.openCalendar") }}
      </NuxtLink>
    </div>
  </div>
</template>
