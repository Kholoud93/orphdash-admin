...
type Row =
  | { key: string; sort: number; kind: "campaign"; date: string; campaign: Campaign }
  | { key: string; sort: number; kind: "donation"; date: string; donation: Donation };

const rows = computed<Row[]>(() => {
  const list: Row[] = [];
  for (const c of orphanStore.campaigns.value) {
    list.push({
      key: `c-${c.id}`,
  ...
  for (const d of orphanStore.donations.value) {
    list.push({
      key: `d-${d.id}`,
    ...
    <QuUISection>
      ...
      <ul v-if="rows.length" class="mt-3 space-y-3">
        <li
          v-for="r in rows"
          :key="r.key"
          class="flex flex-wrap items-start justify-between gap-2 rounded-xl border px-4 py-3 text-sm"
        >
          <div>
            <span v-if="r.kind === 'campaign'">{{
              t("calendar.entryCampaignEnds", { name: r.campaign.name })
            }}</span>
            <span v-else>{{
              t("calendar.entryDonationOn", {
